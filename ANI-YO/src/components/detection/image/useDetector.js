...
const detectFrame = async (model, labels, imageRef, canvasRef) => {
    ...
    // Show alert when an animal is detected
    if (predictions.length > 0) {
        const detectedAnimal = predictions[0].class; // Get detected animal name
        const confidenceScore = (predictions[0].score * 100).toFixed(2); // Convert to percentage
        alert(`Detected: ${detectedAnimal} (Confidence: ${confidenceScore}%)`); //its for image
    }
};
